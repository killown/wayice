...
use crate::{
    core::focus::{KeyboardFocusTarget, PointerFocusTarget},
    shell::WindowElement,
};
#[cfg(feature = "xwayland")]
use smithay::{
...
#[derive(Debug)]
pub struct WayiceState<BackendData: Backend + 'static> {
    pub backend_data: BackendData,
    pub socket_name: Option<String>,
    pub display_handle: DisplayHandle,
    pub running: Arc<AtomicBool>,
    pub handle: LoopHandle<'static, WayiceState<BackendData>>,
    ...
    pub seat_state: SeatState<WayiceState<BackendData>>,
    ...
    pub seat: Seat<WayiceState<BackendData>>,
    pub clock: Clock<Monotonic>,
    pub pointer: PointerHandle<WayiceState<BackendData>>,
...
impl<BackendData: Backend> ServerDndGrabHandler for WayiceState<BackendData> {
    fn send(&mut self, _mime_type: String, _fd: OwnedFd, _seat: Seat<Self>) {
        unreachable!("Wayice doesn't do server-side grabs");
    }
}
delegate_data_device!(@<BackendData: Backend + 'static> WayiceState<BackendData>);

impl<BackendData: Backend> OutputHandler for WayiceState<BackendData> {}
delegate_output!(@<BackendData: Backend + 'static> WayiceState<BackendData>);

impl<BackendData: Backend> SelectionHandler for WayiceState<BackendData> {
...
impl<BackendData: Backend> PrimarySelectionHandler for WayiceState<BackendData> {
    fn primary_selection_state(&self) -> &PrimarySelectionState {
        &self.primary_selection_state
    }
}
delegate_primary_selection!(@<BackendData: Backend + 'static> WayiceState<BackendData>);

impl<BackendData: Backend> DataControlHandler for WayiceState<BackendData> {
    fn data_control_state(&self) -> &DataControlState {
        &self.data_control_state
    }
}

delegate_data_control!(@<BackendData: Backend + 'static> WayiceState<BackendData>);

impl<BackendData: Backend> ShmHandler for WayiceState<BackendData> {
    fn shm_state(&self) -> &ShmState {
        &self.shm_state
    }
}
delegate_shm!(@<BackendData: Backend + 'static> WayiceState<BackendData>);

impl<BackendData: Backend> SeatHandler for WayiceState<BackendData> {
    type KeyboardFocus = KeyboardFocusTarget;
    type PointerFocus = PointerFocusTarget;
    type TouchFocus = PointerFocusTarget;

    fn seat_state(&mut self) -> &mut SeatState<WayiceState<BackendData>> {
        &mut self.seat_state
    }
...
delegate_seat!(@<BackendData: Backend + 'static> WayiceState<BackendData>);

impl<BackendData: Backend> TabletSeatHandler for WayiceState<BackendData> {
    fn tablet_tool_image(&mut self, _tool: &TabletToolDescriptor, image: CursorImageStatus) {
        // TODO: tablet tools should have their own cursors
        self.cursor_status = image;
    }
}
delegate_tablet_manager!(@<BackendData: Backend + 'static> WayiceState<BackendData>);

delegate_text_input_manager!(@<BackendData: Backend + 'static> WayiceState<BackendData>);

impl<BackendData: Backend> InputMethodHandler for WayiceState<BackendData> {
...
delegate_input_method_manager!(@<BackendData: Backend + 'static> WayiceState<BackendData>);

impl<BackendData: Backend> KeyboardShortcutsInhibitHandler for WayiceState<BackendData> {
...
delegate_keyboard_shortcuts_inhibit!(@<BackendData: Backend + 'static> WayiceState<BackendData>);

delegate_virtual_keyboard_manager!(@<BackendData: Backend + 'static> WayiceState<BackendData>);

delegate_pointer_gestures!(@<BackendData: Backend + 'static> WayiceState<BackendData>);

delegate_relative_pointer!(@<BackendData: Backend + 'static> WayiceState<BackendData>);

impl<BackendData: Backend> PointerConstraintsHandler for WayiceState<BackendData> {
...
delegate_pointer_constraints!(@<BackendData: Backend + 'static> WayiceState<BackendData>);

delegate_viewporter!(@<BackendData: Backend + 'static> WayiceState<BackendData>);

impl<BackendData: Backend> XdgActivationHandler for WayiceState<BackendData> {
...
delegate_xdg_activation!(@<BackendData: Backend + 'static> WayiceState<BackendData>);

impl<BackendData: Backend> XdgDecorationHandler for WayiceState<BackendData> {
...
delegate_xdg_decoration!(@<BackendData: Backend + 'static> WayiceState<BackendData>);

delegate_xdg_shell!(@<BackendData: Backend + 'static> WayiceState<BackendData>);
delegate_layer_shell!(@<BackendData: Backend + 'static> WayiceState<BackendData>);
delegate_presentation!(@<BackendData: Backend + 'static> WayiceState<BackendData>);

impl<BackendData: Backend> FractionalScaleHandler for WayiceState<BackendData> {
    fn new_fractional_scale(
        ...
        // of the space (which in case of wayice will also be the output a toplevel will
...
delegate_fractional_scale!(@<BackendData: Backend + 'static> WayiceState<BackendData>);

impl<BackendData: Backend + 'static> SecurityContextHandler for WayiceState<BackendData> {
...
delegate_security_context!(@<BackendData: Backend + 'static> WayiceState<BackendData>);

#[cfg(feature = "xwayland")]
impl<BackendData: Backend + 'static> XWaylandKeyboardGrabHandler for WayiceState<BackendData> {
...
#[cfg(feature = "xwayland")]
delegate_xwayland_keyboard_grab!(@<BackendData: Backend + 'static> WayiceState<BackendData>);

#[cfg(feature = "xwayland")]
delegate_xwayland_shell!(@<BackendData: Backend + 'static> WayiceState<BackendData>);

impl<BackendData: Backend> XdgForeignHandler for WayiceState<BackendData> {
    fn xdg_foreign_state(&mut self) -> &mut XdgForeignState {
        &mut self.xdg_foreign_state
    }
}
smithay::delegate_xdg_foreign!(@<BackendData: Backend + 'static> WayiceState<BackendData>);

smithay::delegate_single_pixel_buffer!(@<BackendData: Backend + 'static> WayiceState<BackendData>);

impl<BackendData: Backend + 'static> WayiceState<BackendData> {
    pub fn init(
        display: Display<WayiceState<BackendData>>,
        handle: LoopHandle<'static, WayiceState<BackendData>>,
        backend_data: BackendData,
        listen_on_socket: bool,
    ) -> WayiceState<BackendData> {

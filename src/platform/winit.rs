...
use crate::core::state::{post_repaint, take_presentation_feedback, Backend, WayiceState};
use crate::{core::drawing::*, core::render::*};
...
impl DmabufHandler for WayiceState<WinitData> {
    ...
    fn dmabuf_imported(&mut self, _global: &DmabufGlobal, dmabuf: Dmabuf, notifier: ImportNotifier) {
        if self
            ...
            .is_ok()
        {
            let _ = notifier.successful::<WayiceState<WinitData>>();
        } else {
            notifier.failed();
        }
    }
}
delegate_dmabuf!(WayiceState<WinitData>);
...
pub fn run_winit() {
    ...
    let _global = output.create_global::<WayiceState<WinitData>>(&display.handle());
    ...
    let dmabuf_state = if let Some(default_feedback) = dmabuf_default_feedback {
        ...
        let dmabuf_global = dmabuf_state.create_global_with_default_feedback::<WayiceState<WinitData>>(
            &display.handle(),
            &default_feedback,
        );
        ...
        let mut dmabuf_state = DmabufState::new();
        let dmabuf_global =
            dmabuf_state.create_global::<WayiceState<WinitData>>(&display.handle(), dmabuf_formats);
    ...
    // Start the IPC server in a non-blocking way using the runtime
    rt.spawn(async {
        if let Err(e) = start_ipc_server().await {

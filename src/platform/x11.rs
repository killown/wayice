...
impl DmabufHandler for WayiceState<X11Data> {
    ...
    fn dmabuf_imported(&mut self, _global: &DmabufGlobal, dmabuf: Dmabuf, notifier: ImportNotifier) {
        if self.backend_data.renderer.import_dmabuf(&dmabuf, None).is_ok() {
            let _ = notifier.successful::<WayiceState<X11Data>>();
        } else {
            notifier.failed();
        }
    }
}
delegate_dmabuf!(WayiceState<X11Data>);
...
pub fn run_x11() {
    ...
    let dmabuf_global = dmabuf_state.create_global_with_default_feedback::<WayiceState<X11Data>>(
        &display.handle(),
    ...
    let _global = output.create_global::<WayiceState<X11Data>>(&display.handle());

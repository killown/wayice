...
use crate::{
    core::drawing::*,
    core::render::*,
    core::state::{post_repaint, take_presentation_feedback, Backend, WayiceState},
    shell::WindowElement,
};
...
impl DmabufHandler for WayiceState<UdevData> {
    ...
    fn dmabuf_imported(&mut self, _global: &DmabufGlobal, dmabuf: Dmabuf, notifier: ImportNotifier) {
            ...
            .is_ok()
        {
            ...
            let _ = notifier.successful::<WayiceState<UdevData>>();
        } else {
            notifier.failed();
        }
    }
}
delegate_dmabuf!(WayiceState<UdevData>);
...
pub fn run_udev() {
    ...
    let data = UdevData {
        ...
        pointer_image: crate::core::cursor::Cursor::load(),
    ...
    let mut state = WayiceState::init(display, event_loop.handle(), data, true);
        ...
        .insert_source(notifier, move |event, &mut (), data| match event {
            ...
            SessionEvent::ActivateSession => {
                    ...
                    .map(|(handle, backend)| (*handle, backend))
                {
                    ...
                    if let Some(lease_global) = backend.leasing_global.as_mut() {
                        lease_global.resume::<WayiceState<UdevData>>();
                    }
    ...
    let global = dmabuf_state
        .create_global_with_default_feedback::<WayiceState<UdevData>>(&display_handle, &default_feedback);
...
impl DrmLeaseHandler for WayiceState<UdevData> {
...
impl Drop for SurfaceData {
    fn drop(&mut self) {
        if let Some(global) = self.global.take() {
            self.dh.remove_global::<WayiceState<UdevData>>(global);
        }
    }
}
...
impl WayiceState<UdevData> {
    fn device_added(&mut self, node: DrmNode, path: &Path) -> Result<(), DeviceAddError> {
        ...
        let registration_token = self
            ...
            .insert_source(
                notifier,
                move |event, metadata, data: &mut WayiceState<_>| match event {
        ...
        self.backend_data.backends.insert(
            ...
            BackendData {
                ...
                leasing_global: DrmLeaseState::new::<WayiceState<UdevData>>(&self.display_handle, &node)
                    .map_err(|err| {
    ...
    fn connector_connected(&mut self, node: DrmNode, connector: connector::Info, crtc: crtc::Handle) {
        ...
        if non_desktop {
            ...
            if let Some(lease_state) = device.leasing_global.as_mut() {
                lease_state.add_connector::<WayiceState<UdevData>>(
                    connector.handle(),
                    output_name,
                    format!("{} {}", make, model),
                );
            ...
            let global = output.create_global::<WayiceState<UdevData>>(&self.display_handle);
    ...
    fn device_removed(&mut self, node: DrmNode) {
        ...
        if let Some(mut backend_data) = self.backend_data.backends.remove(&node) {
            if let Some(mut leasing_global) = backend_data.leasing_global.take() {
                leasing_global.disable_global::<WayiceState<UdevData>>();
            }
    ...
    fn schedule_initial_render(
        ...
        node: DrmNode,
        crtc: crtc::Handle,
        evt_handle: LoopHandle<'static, WayiceState<UdevData>>,
    ) {
        let device = if let Some(device) = self.backend_data.backends.get_mut(&node) {
            device
        } else {
